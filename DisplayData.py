...
def showSPLOM(data, hue=None, hue_order=None, palette=None, vars=None, x_vars=None, y_vars=None, kind='scatter',
              diag_kind='hist', markers=None, size=2.5, aspect=1, dropna=True, plot_kws=None, diag_kws=None,
              grid_kws=None):
    sns.pairplot(data, hue=None, hue_order=hue_order, palette=palette, vars=vars, x_vars=x_vars, y_vars=y_vars,
                 kind=kind, diag_kind=diag_kind, markers=markers, size=size, aspect=aspect, dropna=dropna,
...
def showBins(data, return_type='dict'):
    p = data.boxplot(return_type=return_type)
...
# show test.py line and predict scatter
